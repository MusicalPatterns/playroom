...
describe('submitting pattern spec changes', () => {
    ...
    it('does not submit all the pattern spec inputs when you press enter (only the one you are focused on)', async done => {
        const inputNotToBeSubmitted = await findElement(testGlobals.tab, `input#${PATTERN_SPEC_RANGED_PROPERTY_ONE_KEY}`)
        ...
        const inputToSubmit = await findElement(testGlobals.tab, `input#${PATTERN_SPEC_RANGED_PROPERTY_TWO_KEY}`)
        ...
        ]).toContain(await elementInnerText(`.${SecretSelectorsForTest.SECRET_SUBMITTED_PATTERN_SPEC_INPUT}#${PATTERN_SPEC_RANGED_PROPERTY_TWO_KEY}`))
    ...
    it('does not submit all the pattern spec inputs when you press a submit button (only the one the button is for)', async done => {
        ...
        expect(await elementInnerText(`.${SecretSelectorsForTest.SECRET_SUBMITTED_PATTERN_SPEC_INPUT}#${PATTERN_SPEC_OPTIONED_PROPERTY_ONE_KEY}`))
            .toBe(TEST_PATTERN_SPEC_OPTIONED_PROPERTY_ONE_INITIAL_VALUE)
        expect(await elementInnerText(`.${SecretSelectorsForTest.SECRET_SUBMITTED_PATTERN_SPEC_INPUT}#${PATTERN_SPEC_RANGED_PROPERTY_TWO_KEY}`))
    ...
    it('preserves earlier pattern spec changes you have made when you submit a second pattern spec input', async done => {
        const previouslySubmittedInput = await findElement(testGlobals.tab, `input#${PATTERN_SPEC_RANGED_PROPERTY_ONE_KEY}`)
        await modify(previouslySubmittedInput)

        await testGlobals.page.select(`select#${PATTERN_SPEC_OPTIONED_PROPERTY_ONE_KEY}`, TEST_PATTERN_SPEC_OPTIONED_PROPERTY_ONE_MODIFIED_VALUE)
        await submitSelectByPressingEnter()

        const inputUnderTest = await findElement(testGlobals.tab, `input#${PATTERN_SPEC_RANGED_PROPERTY_TWO_KEY}`)
    ...
    describe('does the same for optioned controls', () => {
        ...
        it('does not submit all the pattern spec selects when you press enter (only the one you are focused on)', async done => {
            ...
            expect(await elementInnerText(`.${SecretSelectorsForTest.SECRET_SUBMITTED_PATTERN_SPEC_INPUT}#${PATTERN_SPEC_RANGED_PROPERTY_ONE_KEY}`))
                .toBe(`${TEST_PATTERN_SPEC_RANGED_PROPERTY_ONE_INITIAL_VALUE}`)
            expect(await elementInnerText(`.${SecretSelectorsForTest.SECRET_SUBMITTED_PATTERN_SPEC_INPUT}#${PATTERN_SPEC_OPTIONED_PROPERTY_TWO_KEY}`))
