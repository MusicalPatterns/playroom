...
describe('reset button', () => {
    ...
    it('if there were any unsubmitted controls, they no longer appear as unsubmitted', async done => {
        const controlToBeUnsubmitted = await findElement(testGlobals.tab, `input[type=number]#${SPEC_RANGED_PROPERTY_ONE_KEY}`)
        ...
        const checkbox = await findElement(testGlobals.tab, `input[type=checkbox]#${SPEC_TOGGLED_PROPERTY_KEY}`)
        await clickElement(checkbox)
        await loseFocus()
        expect(await elementExists(`input[type=checkbox]#${SPEC_TOGGLED_PROPERTY_KEY}.${SpecControlStates.UNSUBMITTED}`))
            .toBeTruthy()

        // await sleep(5000)

        const controlToBeSubmittedSuchThatResetButtonIsEnabled = await findElement(testGlobals.tab, `input[type=number]#${SPEC_RANGED_PROPERTY_TWO_KEY}`)
        ...
        expect(await elementExists(`select#${SPEC_OPTIONED_PROPERTY_ONE_KEY}.${SpecControlStates.VALID_AND_SUBMITTED}`))
            .toBeTruthy()
        expect(await elementExists(`input[type=checkbox]#${SPEC_TOGGLED_PROPERTY_KEY}.${SpecControlStates.VALID_AND_SUBMITTED}`))
            .toBeTruthy()
    ...
    it('if there were any invalid controls, they no longer appear as invalid', async done => {
        ...
        const inputToBeSubmittedSuchThatResetButtonIsEnabled = await findElement(testGlobals.tab, `input[type=number]#${SPEC_RANGED_PROPERTY_TWO_KEY}`)
